...
void test_move_picker() {
    ...
    "move_picker"_test = []() {
        ...
        ut::expect(std::ranges::all_of(good_capture_moves | std::views::elements<1>, [](auto&& eval) {
            return eval.see == +100;
        }));

        auto quiet_moves = move_picker(move_picker_t::QUIET_MOVES);
        ut::expect(std::ranges::is_permutation(quiet_moves | std::views::elements<0> | std::views::take(2), history_moves));
        ut::expect(std::ranges::is_permutation(quiet_moves | std::views::elements<0> | std::views::drop(2), std::initializer_list{"e1e2"_m, "e1f2"_m, "d2d3"_m, "e4c3"_m, "e4g5"_m, "e4g3"_m, "e4f2"_m}));
        ut::expect(std::ranges::all_of(quiet_moves | std::views::elements<1>, [](auto&& eval) {
            return eval.see == 0;
        }));
        ...
        ut::expect(std::ranges::all_of(bad_capture_moves | std::views::elements<1>, [](auto&& eval) {
            return eval.see == -300;
        }));
    };
}
